...
void getControlParametersData(const hexa_package::controlParameters::ConstPtr& msg){
  std::cout<<"Control Parameters"<<endl;
  P_RH = msg->P_RH;
  P_LH = msg->P_LH;
  I_R = msg->I_R;
  C_R = msg->C_R;
...
int main(int argc, char* argv[]) {
  ...
  ros::Subscriber loadCell1Sub = n.subscribe("sensor", 2, getSensorData);
  ros::Subscriber settingsSub = n.subscribe("setting", 1000, getSettingData);
  ros::Subscriber controlParametersSub = n.subscribe("controlParameters", 1000, getControlParametersData);
...
//     cout<<"p0: "<<actual_position_0<<"\tp1: "<<actual_position_1<<endl;
...
static long double PositionActualValueright = 0;
static long double PositionActualValueleft = 0;
...
// while not rospy.is_shutdown():
    long double Force_RH = 0.07*(- 0.023306 * (LoadcellRightHip) + 458.15 - 6.0) - 8;
    long double Force_LH = 0.07*(- 0.023359 * (LoadcellLeftHip) + 756.79 - 6.0) - 1;

   // cout<<"PositionActualValueright: "<<PositionActualValueright<<"\tPositionActualValueleft: "<<PositionActualValueleft<<endl;
    //# Theta
    long double Theta_RH = -(2 * pi / (4800)) * PositionActualValueright;
    long double Theta_LH = +(2 * pi / (4800)) * PositionActualValueleft;
//     cout<<"Theta_RH: "<<Theta_RH<<"\tTheta_LH: "<<Theta_LH<<endl;
...
//     cout << "Force LH: " << Force_LH << "\tForce_RH:" << (Force_RH) << "\n";
